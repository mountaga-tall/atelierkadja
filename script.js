(() => {
  ...
  // ---------------------------------------------------------
  // PWA install prompt — Android/Chromium + iOS Chrome/Safari guidance
  // ---------------------------------------------------------
  ...
  const createInstallBanner = ({ ios = false, fallback = false } = {}) => {
    ...
    banner.innerHTML = `<div class="app-install-icon"><img src="icons/icon-192.png" alt="Logo Atelier Kadja"></div><div class="app-install-copy"><strong>Installer l’app Atelier Kadja</strong>${helper}</div><button type="button" class="app-install-button" data-install-app>${ios || fallback ? 'Comment faire' : 'Installer'}</button><button type="button" class="app-install-close" aria-label="Fermer">×</button>`;
    ...
    banner.querySelector('[data-install-app]')?.addEventListener('click', async () => {
      ...
      if (ios) {
        banner.querySelector('.app-install-copy span').innerHTML = 'Sur iPhone/iPad : touchez <strong>Partager</strong> <strong>▢↑</strong>, puis <strong>Ajouter à l’écran d’accueil</strong>.';
      } else {
